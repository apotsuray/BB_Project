...
public class AlgorithmThrowInOverHalf implements Runnable {
    private final SharedState sharedState;
    private final int threadIndex;
    private static List<CheckingMatchSmalls> checkingMatches;

    public AlgorithmThrowInOverHalf(SharedState sharedState, int threadIndex) {
                ...
                for (FonTournament fonTournament : sharedState.getListTournament()) {
                    if (fonTournament.isHaveSmall()) {
                        for (FonMatch fonMatch : fonTournament.getMatches()) {
                            if (fonMatch.getThrowIn1stTime() != null && fonMatch.getThrowIn1stTime().getTotal().size() != 0) {
                                if (checkingMatches.get(i).tournamentName.equals(fonTournament.getName()) &&
                                    ...
                                    int zapas = (int) fonMatch.getThrowIn1stTime().getTotal().get(0).value;
                                    zapas = zapas - (fonMatch.getThrowIn1stTime().getScore1() + fonMatch.getThrowIn1stTime().getScore2()) + 1;
                                    ...
                                    if (itog < 45 - checkingMatches.get(i).advantage) {
                                        ...
                                        stringBuilder.append("☄️ ")
                                                ...
                                                .append("Ставка: 1-й тайм. Тотал аутов больше ")
                                                .append(fonMatch.getThrowIn1stTime().getTotal().get(0).value).append(" за ")
                                                .append(fonMatch.getThrowIn1stTime().getTotal().get(0).odds).append('\n')
