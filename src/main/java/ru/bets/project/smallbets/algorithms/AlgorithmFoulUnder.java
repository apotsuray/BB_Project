...
public class AlgorithmFoulUnder implements Runnable {
    private final SharedState sharedState;
    private final int threadIndex;
    private static List<CheckingMatchSmalls> checkingMatches;

    public AlgorithmFoulUnder(SharedState sharedState, int threadIndex) {
                ...
                for (FonTournament fonTournament : sharedState.getListTournament()) {
                    if (fonTournament.isHaveSmall()) {
                        for (FonMatch fonMatch : fonTournament.getMatches()) {
                            if (fonMatch.getFoul() != null && !fonMatch.getFoul().getTotal().isEmpty()) {
                                if (checkingMatches.get(i).tournamentName.equals(fonTournament.getName()) &&
                                    ...
                                    int zapas = (int) fonMatch.getFoul().getTotal().get(1).value;
                                    zapas = zapas - (fonMatch.getFoul().getScore1() + fonMatch.getFoul().getScore2());
                                    ...
                                    if (itog - 90 >= checkingMatches.get(i).advantage) {
                                        ...
                                        stringBuilder.append("💀 ")
                                                ...
                                                .append("Ставка: Тотал фолов меньше ")
                                                .append(fonMatch.getFoul().getTotal().get(1).value).append(" за ")
                                                .append(fonMatch.getFoul().getTotal().get(1).odds).append('\n')
