...
public class AlgorithmFoulUnderHalf implements Runnable {
    private final SharedState sharedState;
    private final int threadIndex;
    private static List<CheckingMatchSmalls> checkingMatches;

    public AlgorithmFoulUnderHalf(SharedState sharedState, int threadIndex) {
                            ...
                            for (FonMatch fonMatch : fonTournament.getMatches()) {
                                if (fonMatch.getFoul1stTime() != null && !fonMatch.getFoul1stTime().getTotal().isEmpty()) {
                                    if (checkingMatches.get(i).tournamentName.equals(fonTournament.getName()) &&
                                        ...
                                        int zapas = (int) fonMatch.getFoul1stTime().getTotal().get(1).value;
                                        zapas = zapas - (fonMatch.getFoul1stTime().getScore1() + fonMatch.getFoul1stTime().getScore2());
                                        ...
                                        if (itog - 45 >= checkingMatches.get(i).advantage) {
                                            ...
                                            stringBuilder.append("💀 ")
                                                    ...
                                                    .append("Ставка: 1-й тайм. Тотал фолов меньше ")
                                                    .append(fonMatch.getFoul1stTime().getTotal().get(1).value).append(" за ")
                                                    .append(fonMatch.getFoul1stTime().getTotal().get(1).odds).append('\n')
