...
public class AlgorithmThrowInUnder implements Runnable {
    private final SharedState sharedState;
    private final int threadIndex;
    private static List<CheckingMatchSmalls> checkingMatches;

    public AlgorithmThrowInUnder(SharedState sharedState, int threadIndex) {
                        ...
                        for (FonMatch fonMatch : fonTournament.getMatches()) {
                            if (fonMatch.getThrowIn() != null && !fonMatch.getThrowIn().getTotal().isEmpty()) {
                                if (checkingMatches.get(i).tournamentName.equals(fonTournament.getName()) &&
                                    ...
                                    int zapas = (int) fonMatch.getThrowIn().getTotal().get(1).value;
                                    zapas = zapas - (fonMatch.getThrowIn().getScore1() + fonMatch.getThrowIn().getScore2());
                                    ...
                                    if (itog - 90 >= checkingMatches.get(i).advantage) {
                                        ...
                                        stringBuilder.append("☄️ ")
                                                ...
                                                .append("Ставка: Тотал аутов меньше ")
                                                .append(fonMatch.getThrowIn().getTotal().get(1).value).append(" за ")
                                                .append(fonMatch.getThrowIn().getTotal().get(1).odds).append('\n')
