...
public class AlgorithmCardUnderHalf implements Runnable {
    private final SharedState sharedState;
    private final int threadIndex;
    private static List<CheckingMatch> checkingMatches;

    public AlgorithmCardUnderHalf(SharedState sharedState, int threadIndex) {
                        ...
                        for (FonMatch fonMatch : fonTournament.getMatches()) {
                            if (fonMatch.getYellowCard1stTime() != null && !fonMatch.getYellowCard1stTime().getTotal().isEmpty()) {
                                if (checkingMatches.get(i).tournamentName.equals(fonTournament.getName()) && checkingMatches.get(i).matchName.equals(fonMatch.getName())) {
                                    if ((fonMatch.getYellowCard1stTime().getTotal().get(1).value >= checkingMatches.get(i).value &&
                                            fonMatch.getYellowCard1stTime().getTotal().get(1).odds >= checkingMatches.get(i).odds)
                                            || fonMatch.getYellowCard1stTime().getTotal().get(1).value > checkingMatches.get(i).value) {
                                        StringBuilder stringBuilder = new StringBuilder();
                                        stringBuilder.append("🟨 ")
                                                .append(fonTournament.getName()).append('\n')
                                                .append(fonMatch.getName()).append('\n')
                                                .append("Ставка: 1-й тайм. Тотал желтых карточек меньше ")
                                                .append(fonMatch.getYellowCard1stTime().getTotal().get(1).value).append(" за ")
                                                .append(fonMatch.getYellowCard1stTime().getTotal().get(1).odds).append('\n')
