...
public class AlgorithmGoalKickUnderHalf implements Runnable {
    private final SharedState sharedState;
    private final int threadIndex;
    private static List<CheckingMatchSmalls> checkingMatches;

    public AlgorithmGoalKickUnderHalf(SharedState sharedState, int threadIndex) {
                        ...
                        for (FonMatch fonMatch : fonTournament.getMatches()) {
                            if (fonMatch.getGoalKick1stTime() != null && !fonMatch.getGoalKick1stTime().getTotal().isEmpty()) {
                                if (checkingMatches.get(i).tournamentName.equals(fonTournament.getName()) &&
                                    ...
                                    int zapas = (int) fonMatch.getGoalKick1stTime().getTotal().get(1).value;
                                    zapas = zapas - (fonMatch.getGoalKick1stTime().getScore1() + fonMatch.getGoalKick1stTime().getScore2());
                                    ...
                                    if (itog - 45 >= checkingMatches.get(i).advantage) {
                                        ...
                                        stringBuilder.append("🥅 ")
                                                ...
                                                .append("Ставка: 1-й тайм. Тотал уотв меньше ")
                                                .append(fonMatch.getGoalKick1stTime().getTotal().get(1).value).append(" за ")
                                                .append(fonMatch.getGoalKick1stTime().getTotal().get(1).odds).append('\n')
