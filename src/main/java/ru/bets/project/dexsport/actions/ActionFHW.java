...
public class ActionFHW extends Action{
    @Override
    public void execute(List<WebElement> buttonOdds, MatchFootballDEX match) {
        ...
        match.getCard1half().setWin1(Double.parseDouble(oddHome));
        match.getCard1half().setWin2(Double.parseDouble(oddAway));
        match.getCard1half().setDraw(Double.parseDouble(oddDraw));
    }
}
